...
in float aImageSize;
in float aImageBorderRadius;

out vec2 vUvs;
out float vAlpha;
flat out int vInstanceId;
out float vImageSize;
out float vImageBorderRadius;
...
void main() {
    ...
    vImageBorderRadius = aImageBorderRadius;
}
`;
...
void main() {
    ...
    st = (st - 0.5) * vImageSize + 0.5;
    st = st * cellSize + cellOffset;
    st = clamp(st, 0.0, 1.0);

    vec4 texColor = texture(uTex, st);
    
    // Apply circular clipping to ensure it's always a circle
float distToCenter = length(vUvs - vec2(0.5));
float alpha_mask = smoothstep(0.5, 0.5 - vImageBorderRadius, distToCenter);
texColor.a *= alpha_mask;
    texColor.a *= vAlpha;
    outColor = texColor;
}
`;
...
class InfiniteGridMenu {
  ...
  private discLocations!: {
    ...
    uUpdateSignal: WebGLUniformLocation | null;
  };
  ...
  private discInstances!: {
    ...
    imageBorderRadii: Float32Array;
    matrixBuffer: WebGLBuffer | null;
    imageSizeBuffer: WebGLBuffer | null;
    imageBorderRadiusBuffer: WebGLBuffer | null;
  };
  ...
  private DISC_INITIAL_SCALE: number;
  private defaultImageSize: number;
  private defaultImageBorderRadius: number;
  ...
  constructor(
    ...
    discInitialScale: number = 0.15,
    defaultImageSize: number = 1.0,
    defaultImageBorderRadius: number = 0.0
  ) {
    this.SPHERE_RADIUS = sphereRadius;
    this.DISC_INITIAL_SCALE = discInitialScale;
    this.defaultImageSize = defaultImageSize;
    this.defaultImageBorderRadius = defaultImageBorderRadius;
    this.init(onInit);
  }
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discLocations = {
      ...
      aImageSize: gl.getAttribLocation(this.discProgram!, "aImageSize"),
      aImageBorderRadius: gl.getAttribLocation(
        this.discProgram!,
        "aImageBorderRadius"
      ),
      ...
      uUpdateSignal: gl.getUniformLocation(this.discProgram!, "uUpdateSignal"),
    };

    this.discGeo = new DiscGeometry();
  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // Draw image centrally, maintaining aspect ratio, without stretching
        const aspectRatio = img.width / img.height;
        let drawWidth = cellSize;
        let drawHeight = cellSize;

        if (aspectRatio > 1) drawHeight = cellSize / aspectRatio;
        else drawWidth = cellSize * aspectRatio;
  ...
  private initDiscInstances(count: number): void {
    ...
    const imageSizes = new Float32Array(count);
    const imageBorderRadii = new Float32Array(count);
    const matrixBuffer = gl.createBuffer();
    const imageSizeBuffer = gl.createBuffer();
    const imageBorderRadiusBuffer = gl.createBuffer();
    ...
    this.discInstances = {
      ...
      matrixBuffer,
      imageSizeBuffer,
      imageBorderRadiusBuffer,
    };

    gl.bindVertexArray(this.discVAO);

    gl.bindBuffer(gl.ARRAY_BUFFER, this.discInstances.matrixBuffer);
    gl.bufferData(
      gl.ARRAY_BUFFER,
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.discInstances.imageSizeBuffer);
    gl.bufferData(
      gl.ARRAY_BUFFER,
      this.discInstances.imageSizes.byteLength,
      gl.DYNAMIC_DRAW
    );
    gl.enableVertexAttribArray(this.discLocations.aImageSize);
    gl.vertexAttribPointer(
      this.discLocations.aImageSize,
      1,
      gl.FLOAT,
      false,
      0,
    ...
    gl.vertexAttribDivisor(this.discLocations.aImageSize, 1);

    gl.bindBuffer(gl.ARRAY_BUFFER, this.discInstances.imageBorderRadiusBuffer);
    gl.bufferData(
      gl.ARRAY_BUFFER,
      this.discInstances.imageBorderRadii.byteLength,
      gl.DYNAMIC_DRAW
    );
    gl.enableVertexAttribArray(this.discLocations.aImageBorderRadius);
    gl.vertexAttribPointer(
      this.discLocations.aImageBorderRadius,
      1,
      gl.FLOAT,
      false,
      0,
    ...
    gl.vertexAttribDivisor(this.discLocations.aImageBorderRadius, 1);
  ...
  private animate(deltaTime: number): void {
    ...
    positions.forEach((p, ndx) => {
      ...
      const finalScale = s * this.DISC_INITIAL_SCALE * imageScaleFactor;
      ...
      const itemIndex = ndx % this.items.length;
      const item = this.items[itemIndex];
      this.discInstances.imageSizes[ndx] =
        item.imageSize ?? this.defaultImageSize;
      this.discInstances.imageBorderRadii[ndx] =
        item.imageBorderRadius ?? this.defaultImageBorderRadius;
    });

    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.discInstances.matrixBuffer);
    ...
    this.gl.bindBuffer(
      this.gl.ARRAY_BUFFER,
      this.discInstances.imageSizeBuffer
    );
    this.gl.bufferSubData(
      this.gl.ARRAY_BUFFER,
      0,
      this.discInstances.imageSizes
    );

    this.gl.bindBuffer(
      this.gl.ARRAY_BUFFER,
      this.discInstances.imageBorderRadiusBuffer
    );
    this.gl.bufferSubData(
      this.gl.ARRAY_BUFFER,
      0,
      this.discInstances.imageBorderRadii
    );
  ...
  private render(): void {
    ...
    gl.uniform1f(this.discLocations.uUpdateSignal, this.updateSignal);
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({
  ...
  defaultImageSize = 1.0,
  defaultImageBorderRadius = 0.0,
}) => {
